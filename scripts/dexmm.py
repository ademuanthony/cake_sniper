...
def buy(triggerAddress, account, amountIn):
    ...
    buyTx = trigger.swapExactETHForTokens(web3.toChecksumAddress(
        bunny), amountIn, amountOutMin, {"from": me, "gas_limit": 750000})
    if buyTx.status == 1:
...
def sell(triggerAddress, account, sellOf):
    ...
    me = account
    trigger = interface.ITrigger2(triggerAddress)
    sellTx = trigger.swapTokensForExactETH(web3.toChecksumAddress(
        bunny), amountIn, 0, {"from": me, "gas_limit": 750000})
    if sellTx.status == 1:
...
def runMarketMakers():
    ...
    while True:
        ...
        try:
            ...
            print(
                f'{currentTime()} - Trigger {caller["address"]} action failed, will rest for a while and try again - {str(e)}')

        waitTime = random.randint(5, 45)
        print(
            f'{currentTime()} - sleeping for {waitTime}\n------------------------------\n')
        time.sleep(waitTime)
...
def retrieveFunds():
    ...
    for seller in sellers:
        ...
        if triggerBalance == 0:
            continue
        tx = trigger.emmergencyWithdrawTkn(tokenAddress, triggerBalance, {
                                           "from": me, "gas_limit": 750000})
        if tx.status == 1:
...
def fundTriggersFromSender():
    ...
    for seller in sellers:
        ...
        if senderBalance == 0:
            continue
        tx = interface.ERC20(tokenAddress).transfer(
            seller["trigger"], senderBalance, {"from": me, "gas_limit": 750000})
        if tx.status == 1:
...
def deployTrigger():
    ...
    dispenser = accounts.load('dex-owner')
    ...
    for num in range(0, int(numberOfTrigger)):
        try:
            ...
            print(
                f'{currentTime()} - new account raw card {new_account["pk"]}')
            ...
            tx = dispenser.transfer(
                ...
                allow_revert=True)
            print(
                f'{currentTime()} - bee {dispenser.address} --> paid {tx.value / 10**18} BNB to new_account')

            trigger = Trigger2.deploy({"from": me})
            new_account["trigger"] = trigger.address
            print(
                f"{currentTime()} - trigger created with the address {trigger.address}")
...
def viewTriggerBalance():
    ...
    for seller in sellers:
        ...
        print(
            f'{currentTime()} - {seller["trigger"]} DFC balance: {dfcBalance/1e8}; BNB balance: {bnbBalance/1e18}\n')
...
def reInitVolumeGenerators():
    ...
    for caller in callers:
        ...
        if caller["volGen"]:
            ...
            if dfcBalance > 0:
                generator.emmergencyWithdrawTkn(DFC_ADDRESS, dfcBalance, {
                                                "from": me, "gas_limit": 750000})

            bnbBalance = wbnb.balanceOf(caller["volGen"])
            if bnbBalance > 0:
                generator.emmergencyWithdrawTkn(WBNB_ADDRESS, bnbBalance, {
                                                "from": me, "gas_limit": 750000})
...
def approveRouter():
    ...
    for caller in callers:
        ...
        generator.approveRouter(WBNB_ADDRESS, 10000000000000000000000000000, {
                                "from": me, "gas_limit": 750000})
        generator.approveRouter(DFC_ADDRESS, 10000000000000000000000000000, {
                                "from": me, "gas_limit": 750000})
...
def generateVolume():
    ...
    while True:
        ...
        if bnbBalance == 0:
            print(
                f"{currentTime()} - BNB balance of {caller['trigger']} is 0. Skipping in 15 seconds")
            time.sleep(15)
            continue
        amountIn = random.randint(70, 98) * bnbBalance/100
        volGen.swapExactETHForTokens(DFC_ADDRESS, amountIn, {
                                     "from": me, "gas_limit": 750000})
