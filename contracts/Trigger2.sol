...
contract Trigger2 is Ownable {
    ...
    function swapExactETHForTokens(address tokenOut, uint256 amountIn)
        external
        returns (bool success)
    {
        require(
            authenticatedSeller[msg.sender] == true,
            "in: must be called by authenticated seller"
        );
        require(
        ...
        IERC20(tokenOut).approve(sandwichRouter, amountIn);
        path[0] = tokenOut;
        path[1] = wbnb;
