		...
		if result.Status == 0 {
			...
			if err != nil {
				...
				if tokenBalance.Int64() == 0 {
					...
					true, true, oldBalanceTrigger, gasPriceFront, swapData.Token)
				}
			}
			sendBackRunningTx(nonce, common.Big1.Mul(global.STANDARD_GAS_PRICE, big.NewInt(2)), oldBalanceTrigger,
			 client, signedFrontrunningTx.Hash(), tx.Hash(), swapData)
		}
	}
...
func emmmergencyCancel(nonce uint64, client *ethclient.Client, gasPriceFront, 
	...
	FirstConfirmed chan *SandwichResult, swapData UniswapExactETHToTokenInput) {
	...
	if firstTxConfirmed == signedCancelTx.Hash() {
		...
		_buildCancelAnalytics(victimHash, signedCancelTx.Hash(), client, oldBalanceTrigger, signedCancelTx.GasPrice(), swapData.Token)
	} else {
...
func sendBackRunningTx(nonce uint64, gasPriceFront, oldBalanceTrigger *big.Int, client *ethclient.Client,
	 frontrunHash, victimHash common.Hash, swapData UniswapExactETHToTokenInput) {
	...
	if result.Status == 0 {
		...
		_buildFrontrunAnalytics(victimHash, frontrunHash, signedBackrunningTx.Hash(), client, false, true, oldBalanceTrigger, signedBackrunningTx.GasPrice(), swapData.Token)
		log.Fatalln()
	} else {
		...
		_buildFrontrunAnalytics(victimHash, frontrunHash, signedBackrunningTx.Hash(), client, false, false,
		 oldBalanceTrigger, signedBackrunningTx.GasPrice(), swapData.Token)
	}
}
